...
struct trackball
{
	...
	void begin_track( const mat4& view_matrix, vec2 m );
	void end_track() { b_tracking = false; }
	void begin_zoom(const mat4& view_matrix, vec2 m);
	void end_zoom() { b_zooming = false; }
	void begin_pan(const mat4& view_matrix, vec2 m);
	void end_pan() { b_panning = false; }
	mat4 update_track( vec2 m ) const;
	mat4 update_zoom( vec2 m ) const;
	mat4 update_pan( vec2 m ) const;
};

inline void trackball::begin_track( const mat4& view_matrix, vec2 m )
{
...
inline void trackball::begin_pan(const mat4& view_matrix, vec2 m)
{
...
inline mat4 trackball::update_zoom(vec2 m) const
{
	...
	mat4 zoom_matrix = view_matrix0;
	zoom_matrix[11] *= p1.y + 1;
...
inline mat4 trackball::update_pan(vec2 m) const
{
	...
	p1 *= scale;	
	p1 = vec3(p1.x, p1.y, sqrtf(std::max(0.0f, 1.0f - length2(p1)))).normalize();	// back-project z=0 onto the unit sphere// apply rotation scale

	mat4 pan_matrix = view_matrix0;
	pan_matrix[3] += p1.x * 100.0f;
	pan_matrix[7] += p1.y * 100.0f;
