...
inline const char* absolute_path( const char* file_path )
{
	...
	char f[_MAX_PATH]; return strdup(_fullpath( f, t, _MAX_PATH )); // canonicalize the path
#else
...
inline const char* cg_conf_path()
{
	...
	if(!conf_path[0])
	{
		...
		if(access(conf_dir,0)!=0&&_mkdir( conf_dir )!=0) printf( "error: failed in mkdir(%s)\n", conf_dir );
...
inline GLFWwindow* cg_create_window( const char* name, int& width, int& height, int version_major=gl_version_t::major_default, int version_minor=gl_version_t::minor_default, bool show_window=true )
{
	...
	if(fp){ ivec2 r; if(2==fscanf( fp, "window_pos_x = %d\nwindow_pos_y = %d\n", &r.x, &r.y )) win_pos=r; fclose(fp); }
#endif
...
inline bool cg_init_extensions( GLFWwindow* window )
{
	...
	if(strGLSLver)
	{
		ivec2 t; if(2==sscanf( strGLSLver, "%d.%d", &t.x, &t.y ))
		{
			v.sl.major = t.x; v.sl.minor = t.y;
			while(v.sl.minor>=10) v.sl.minor/=10;
		}
	}
#endif
...
inline uint cg_create_vertex_array( uint vertex_buffer, uint index_buffer=0 )
{
	...
	for( size_t k=0, byte_offset=0; k<std::extent<decltype(attrib_sizes)>::value; byte_offset+=attrib_sizes[k], k++ )
	{
		...
		glVertexAttribPointer( GLuint(k), attrib_sizes[k]/sizeof(GLfloat), GL_FLOAT, GL_FALSE, sizeof(vertex), (GLvoid*) byte_offset );
	}
...
inline image* cg_load_image( const char* image_path )
{
	...
	for( int y=0; y<h; y++ )
	{
		unsigned char* dst = i->ptr+(h-1-y)*stride1;
